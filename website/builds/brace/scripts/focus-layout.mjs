...
try{
  for(const [width,height] of viewports){
    ...
    const count=await page.locator('a[href],button:not([disabled]),summary,[tabindex]:not([tabindex="-1"])').evaluateAll((nodes)=>nodes.filter((node)=>{const rect=node.getBoundingClientRect(),style=getComputedStyle(node);return rect.width>0&&rect.height>0&&style.display!=="none"&&style.visibility!=="hidden"}).length);
