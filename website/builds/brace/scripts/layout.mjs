...
const targets = [
  ...[[1600,1000],[1440,900],[1366,768],[1024,768],[900,900],[760,900],[430,932],[390,844]].map(([width,height]) => ({path:"/",kind:"main",width,height})),
  ...[[1440,900],[1024,768],[860,900],[620,900],[390,844]].map(([width,height]) => ({path:"/guide/",kind:"guide",width,height})),
];
const pages = [];
try {
  for (const target of targets) {
    ...
    const issues = await page.evaluate(({kind}) => {
      ...
      const selectors=kind==="main"?[".site-bar",".film-stage",".story-line",".product-stage",".download-shell","footer"]:[".guide-bar",".guide-hero .guide-wrap",".guide-layout",".guide-article",".guide-shot"];
      ...
      if(kind==="main"){
        ...
        if(stage&&Math.abs(stage.getBoundingClientRect().height-innerHeight)>3)found.push("film stage is not one viewport");
    ...
    for(const selector of target.kind==="main"?["#story","#product","#download"]:["#first-run","#recall","#privacy"]){
