...
test("tagged releases require pinned GitHub/Sigstore provenance", () => {
...
test("distribution docs distinguish provenance from publisher signing", () => {
  assert.match(distribution, /gh attestation verify BRACE-Setup-0\.7\.0\.exe/);
  ...
  assert.match(distribution, /does not claim code-signing, notarization, or reproducible byte-for-byte builds/);
});
