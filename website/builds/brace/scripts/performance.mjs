...
// Create and discard a blank renderer so the first scenario does not measure Chromium startup; this never contacts BRACE.
const rendererWarmup = await browser.newPage();
await rendererWarmup.goto("about:blank");
await rendererWarmup.close();

const scenarios = [
  {name:"home-desktop", path:"/", viewport:{width:1440,height:900}, maxFcp:4500, maxLcp:4500, maxDom:800},
  {name:"home-mobile", path:"/", viewport:{width:390,height:844}, maxFcp:3500, maxLcp:3500, maxDom:800},
  {name:"guide-mobile", path:"/guide/", viewport:{width:390,height:844}, maxFcp:3500, maxLcp:3500, maxDom:1000},
];
...
try {
  for (const scenario of scenarios) {
    ...
    const numericKeys = ["fcp", "lcp", "cls", "maxLongTask", "longTaskCount", "domNodes", "domContentLoaded", "load"];
    const metrics = Object.fromEntries(numericKeys.map((key) => [key, median(samples.map((sample) => sample[key]))]));
    const requests = Math.max(...samples.map((sample) => sample.requests));
    const jsBytes = Math.max(...samples.map((sample) => sample.jsBytes));
    const cssBytes = Math.max(...samples.map((sample) => sample.cssBytes));
    const consoleErrors = [...new Set(samples.flatMap((sample) => sample.consoleErrors))];
    const httpErrors = [...new Set(samples.flatMap((sample) => sample.httpErrors))];
    const issues = [];
    ...
    if (requests > budgets.maxRequests) issues.push(`requests ${requests} > ${budgets.maxRequests}`);
    ...
    const result = {
      ...
      httpErrors,
      issues,
      samples:samples.map((sample) => Object.fromEntries(Object.entries(sample).map(([key,value]) => [key, typeof value === "number" ? Number(value.toFixed(2)) : value]))),
    };
    results.push(result);
    process.stdout.write(`${issues.length ? "FAIL" : "PASS"} performance ${scenario.name} median(${sampleCount}) FCP=${Math.round(metrics.fcp)} LCP=${Math.round(metrics.lcp)} CLS=${metrics.cls.toFixed(3)} long=${Math.round(metrics.maxLongTask)}ms requests=${requests}\n`);
  }
} finally {
  await browser.close();
}
