...
try {
  for (const target of targets) {
    ...
    if(target.kind==="main")await page.waitForFunction(()=>document.documentElement.dataset.braceExperience==="living-v9");
    const issues = await page.evaluate(({kind}) => {
      ...
      const selectors=kind==="main"?[".site-bar",".film-stage",".story-line",".brace-live-window",".product-stage",".download-shell","footer"]:[".guide-bar",".guide-hero .guide-wrap",".guide-layout",".guide-article",".guide-shot"];
    ...
    for(const selector of target.kind==="main"?["#story","#live-preview","#product","#download"]:["#first-run","#recall","#privacy"]){
