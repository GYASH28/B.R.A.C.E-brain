...
test("production IPC accepts only the BRACE main frame", () => {
  assert.equal(
    isTrustedIpcSender(eventFor("brain://app/index.html"), {
      development: false,
  ...
  assert.equal(
    isTrustedIpcSender(eventFor("brain://app/index.html"), {
      development: false,
      expectedWebContentsId: 8,
    }),
    false,
  );
  assert.throws(
    () => assertTrustedIpcSender(eventFor("https://example.com/"), { development: false }),
    /untrusted/i,
  );
});
...
test("IPC schemas reject malformed and oversized privileged calls", () => {
  ...
  assert.throws(
    () => validateIpcArguments("brace:get-snapshot", ["unexpected"]),
  ...
  assert.throws(
    () => validateIpcArguments("brace:copy-text", ["x".repeat(200_001)]),
    /Invalid BRACE IPC request/,
  );
  assert.deepEqual(
    validateIpcArguments("brace:install-connector", ["generic", "remember"]),
    ["generic", "remember"],
  );
});
