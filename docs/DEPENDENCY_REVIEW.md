...
The 0.3.0 lockfile is the authoritative dependency inventory. `npm ci` verifies its integrity and `npm audit --audit-level=high` blocks known high or critical advisories in CI and release jobs.
...
npm sbom --sbom-format cyclonedx > brace-0.3.0.cdx.json
```
