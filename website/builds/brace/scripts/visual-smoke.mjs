...
async function capture(name, viewport, route, setup, reducedMotion = "no-preference") {
...
try {
  ...
  await capture("home-hero-mobile", { width: 390, height: 844 }, "/", readyHome);
  await capture("home-provenance-meeting", { width: 1440, height: 900 }, "/", async (page) => {
    await readyHome(page);
    await page.locator('[data-source="meeting"]').click();
  });
  await capture("home-brain-graph", { width: 1440, height: 900 }, "/", async (page) => {
    await readyHome(page);
    await page.locator("#atlas").evaluate((node) => node.scrollIntoView({ block: "start", behavior: "instant" }));
    await page.locator('[data-node="maya"]').click();
    await page.waitForTimeout(180);
  });
  await capture("home-company-executive", { width: 1440, height: 900 }, "/", async (page) => {
    await readyHome(page);
    await page.locator("#company").scrollIntoViewIfNeeded();
    await page.locator('[data-role="executive"]').click();
  });
  await capture("home-product", { width: 1440, height: 900 }, "/", async (page) => {
    await readyHome(page);
    await page.locator("#product").evaluate((node) => node.scrollIntoView({ block: "start", behavior: "instant" }));
    await page.waitForTimeout(180);
  });
  await capture("home-hero-reduced", { width: 1440, height: 900 }, "/", readyHome, "reduce");
  await capture("guide-hero-desktop", { width: 1440, height: 900 }, "/guide/", readyGuide);
  await capture("guide-hero-mobile", { width: 390, height: 844 }, "/guide/", readyGuide);
