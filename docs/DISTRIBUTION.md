...
BRACE 0.1.0 targets 64-bit Linux and Windows.

## Release artifacts

| Platform | Artifact | Packaging |
| --- | --- | --- |
| Linux | `BRACE-0.1.0.AppImage` | Portable AppImage |
| Debian / Ubuntu | `brace-brain_0.1.0_amd64.deb` | Debian package |
| Windows | `BRACE Setup 0.1.0.exe` | Per-user NSIS installer |

Tagged releases also include `SHA256SUMS.txt`.
They also include a CycloneDX dependency SBOM named `brace-0.1.0.cdx.json`.
...
```bash
npm run electron:smoke -- "dist/installer/BRACE-0.1.0.AppImage"
```
...
The 0.1.0 installer is not code-signed. Users should verify its SHA-256 digest and expect Windows SmartScreen to show an unknown-publisher warning.
...
0.1.0 does not claim code-signing, notarization, or reproducible byte-for-byte builds. GitHub Actions records the source ref and native runner logs. Future releases should add signing, an SBOM, and artifact attestations before introducing automatic updates.
