...
The published `v0.7.0` preview includes `SHA256SUMS.txt` and the CycloneDX dependency SBOM `brace-0.7.0.cdx.json`. It was published on September 1, 2026, before BRACE added GitHub artifact attestations, so those existing 0.7.0 assets do **not** have a BRACE provenance bundle and must not be described as attested.

New release tags produced after the attestation workflow is merged also include a signed GitHub/Sigstore provenance bundle named `brace-<version>.provenance.json`.
...
| GitHub/Sigstore SLSA provenance for final release subjects | Required for newly tagged releases | Required for newly tagged releases |
...
The published 0.7.0 installer is not code-signed. Users should verify its SHA-256 digest and expect Windows SmartScreen to show an unknown-publisher warning. For a newer release that includes BRACE provenance, verify both the SHA-256 digest and the GitHub artifact attestation.
...
For new tags created by the current release workflow, release subjects are bound to the GitHub Actions release workflow through a signed SLSA provenance attestation generated by `actions/attest`. The action uses a short-lived OIDC identity and Sigstore certificate and stores the attestation with GitHub; the publish job also attaches the generated Sigstore bundle to the release for inspection and archival.

After downloading an asset from one of those attested releases, verify its SHA-256 digest against `SHA256SUMS.txt`, then verify the attestation with GitHub CLI. Replace `X.Y.Z` with the version you actually downloaded:

```bash
gh attestation verify ./BRACE-Setup-X.Y.Z.exe \
...
1. Update the application version, `CHANGELOG.md`, release notes, security/privacy disclosures, and version-specific documentation.
...
5. Tag that verified `main` commit with a **new** matching version (for example `v0.7.1` after updating the package version); never move or reuse the published `v0.7.0` tag.
...
8. Run the released Windows installer smoke workflow against the newly published tag to verify checksum, install, launch, MCP entry point, and uninstall behavior.
