(() => {
  ...
  function updateScroll() {
    ...
    if (side && sideRail && innerWidth > 700) {
      ...
      all('[data-side-panel]', side).forEach((panel, i) => {
        const count = all('[data-side-panel]', side).length;
        const center = i / Math.max(1, count - 1);
  ...
  if (hero && finePointer.matches && !reduceMotion.matches) {
    ...
    hero.addEventListener('pointerleave', () => {
      hero.style.setProperty('--mx', '.5'); hero.style.setProperty('--my', '.5');
  ...
  let chatIndex = 1, chatTimer = 0, chatBusy = false;
  function scrollChat(){ if(stream) stream.scrollTo({top:stream.scrollHeight,behavior:reduceMotion.matches?'auto':'smooth'}); }
  function userMessage(text){ const el=document.createElement('article');el.className='message message--user';const p=document.createElement('p');p.textContent=text;el.appendChild(p);stream?.appendChild(el);scrollChat(); }
  function braceMessage(text,source){ const el=document.createElement('article');el.className='message message--brace';const p=document.createElement('p');p.textContent=text;const f=document.createElement('footer');const s=document.createElement('span');s.textContent=source;const b=document.createElement('b');b.textContent='verified';f.append(s,b);el.append(p,f);stream?.appendChild(el);scrollChat(); }
  function typing(show){ by('.typing',stream)?.remove();if(!show||!stream)return;const el=document.createElement('span');el.className='typing';el.innerHTML='<i></i><i></i><i></i>';stream.appendChild(el);scrollChat(); }
  function trimChat(){ if(!stream)return;let messages=all('.message',stream);while(messages.length>8){messages[0].remove();messages=all('.message',stream);} }
  function runScript(item,manual=false){ if(!item||chatBusy||!stream)return;chatBusy=true;state&&(state.textContent='LISTENING');userMessage(item.q);setTimeout(()=>{typing(true);state&&(state.textContent='RECALLING');setTimeout(()=>{typing(false);braceMessage(item.a,item.source);trimChat();state&&(state.textContent='READY');announcer&&(announcer.textContent=`BRACE recalled: ${item.a}`);chatBusy=false;if(!manual)scheduleChat();},reduceMotion.matches?120:900);},reduceMotion.matches?80:430); }
  function scheduleChat(){ clearTimeout(chatTimer);chatTimer=setTimeout(()=>{if(!document.hidden&&!chatBusy){runScript(scripts[chatIndex%scripts.length]);chatIndex+=1;}else scheduleChat();},3600); }
  all('[data-chat-prompt]').forEach(button=>button.addEventListener('click',()=>{const map={decision:scripts[1],source:scripts[4],scope:scripts[2]};clearTimeout(chatTimer);runScript(map[button.dataset.chatPrompt],true);scheduleChat();}));
  form?.addEventListener('submit',(event)=>{event.preventDefault();const text=input?.value.trim();if(!text||chatBusy)return;input.value='';const lower=text.toLowerCase();const item=lower.includes('source')?{q:text,a:scripts[4].a,source:scripts[4].source}:lower.includes('see')||lower.includes('access')||lower.includes('scope')?{q:text,a:scripts[2].a,source:scripts[2].source}:lower.includes('release')?{q:text,a:scripts[5].a,source:scripts[5].source}:{q:text,a:'I found related local context. In the real app, BRACE would return the matching memory and its evidence instead of inventing an answer.',source:'Local memory demo'};clearTimeout(chatTimer);runScript(item,true);scheduleChat();});
  document.addEventListener('visibilitychange',()=>{if(document.hidden)clearTimeout(chatTimer);else scheduleChat();});
  scheduleChat();

  const brainStories={source:{kind:'SOURCE',title:'Architecture Decisions.md',copy:'Canonical source for the provider boundary and local-first adapter rules.',source:'Selected project file',scope:'Private workspace'},memory:{kind:'MEMORY',title:'Graph-first workspace',copy:'Make the graph the central workspace and keep evidence visible beside every relationship.',source:'Architecture Decisions.md',scope:'Private workspace'},evidence:{kind:'EVIDENCE',title:'Provider boundary receipt',copy:'The retained memory still points back to the exact source that justified it.',source:'Provider data flow.md',scope:'Selected context'}};
  all('[data-brain-hotspot]').forEach(button=>button.addEventListener('click',()=>{const story=brainStories[button.dataset.brainHotspot];if(!story)return;all('[data-brain-hotspot]').forEach(node=>node.classList.toggle('is-active',node===button));by('[data-brain-kind]')&&(by('[data-brain-kind]').textContent=story.kind);by('[data-brain-title]')&&(by('[data-brain-title]').textContent=story.title);by('[data-brain-copy]')&&(by('[data-brain-copy]').textContent=story.copy);by('[data-brain-source]')&&(by('[data-brain-source]').textContent=story.source);by('[data-brain-scope]')&&(by('[data-brain-scope]').textContent=story.scope);}));
